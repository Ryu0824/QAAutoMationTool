...
## 트리거 키워드 (Claude 자동화 명령어)

사용자가 아래 키워드를 입력하면 Claude는 해당 워크플로우를 자동 실행한다.

### `/ship` — 테스트 통과 후 커밋·푸시

트리거: 사용자가 `/ship` 또는 `ship it` 또는 `배포해` 입력 시

실행 순서:
1. `npm test` 실행
2. 테스트 **전부 통과** 시 → `git add -p` 대신 변경 파일 목록 확인 후 스테이징, 커밋 메시지 자동 생성, `git push`
3. 테스트 **실패** 시 → 즉시 중단, 실패 내역 보고 (커밋·푸시 금지)
...
> **주의**: 현재 브랜치가 `main`이면 푸시 전 사용자에게 확인을 구한다 (main 직접 푸시 방지).
